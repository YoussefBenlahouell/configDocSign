...
@Component({
  ...
  styleUrls: ["./all-template.component.sass"],
})
export class AllTemplateComponent implements OnInit {
  heading = "All Templates List";
  subheading = " ";
  ...
  getdata() {
    this.templateService
      .getTemplatesCreatedAndSharedWithMe(
  ...
  oncConfirmShare() {
    ...
    this.selectedUser.forEach((user) => {
      ...
      this.templateService.addShareWith(sharewith).subscribe(
        (x) => console.log(x),
        (err) => console.log(err),
        () => () => window.location.reload()
      );
