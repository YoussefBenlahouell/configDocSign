...
<@layout.registrationLayout displayInfo=social.displayInfo displayWide=(realm.password && social.providers??); section>
    ...
    <#if section = "header">  
      ...
      <div style ="font-weight: 900;">   ${msg("doLogIn")}</div>
        <div style="color :#1a87b8 ;font-size: 14px; line-height:100%;font-family:cursive; padding-top:15px;">   ${msg("loginInstruction")}</div>
    <#elseif section = "form">
                ...
                <#if realm.password>
                    <form id="kc-form-login" onsubmit="login.disabled = true; return true;" action="${url.loginAction}" method="post">
                        ...
                        <div class="${properties.kcFormGroupClass!}">
                            ...
                            <div class="mdc-text-field mdc-text-field--with-leading-icon ${properties.kcLabelClass!}">
                                <i class="material-icons mdc-text-field__icon" role="button">lock</i>
                                   <div style="width: 0px;">
                                  <label style=" position: relative; left:415px;top: 15px; width: 24px; height: 24px; opacity: 0.54; border: none;" id="v" onclick="togglePassword()"><img id="vi" src="${url.resourcesPath}/img/eye-off.png"></label>
                                  </div>
                        ...
                        <div class="${properties.kcFormGroupClass!} ${properties.kcFormSettingClass!}">
                            ...
                            <div class="${properties.kcFormOptionsWrapperClass!}">
                                <#if realm.resetPasswordAllowed>
                                    <span> <a style="position: relative; margin-bottom:60px; left: 322px;top: 20px; width: 21px; height: 24px;opacity: 0.54;border: none;color: darkslateblue;text-size-adjust: 10px;" tabindex="0" href="${url.loginResetCredentialsUrl}">${msg("doForgotPassword")}</a></span>
                                </#if>
                            </div>
                        </div>
                        ...
                        <div class="mdc-card__action-icons">
                            <div  style="margin-top : 18px "class="mdc-card__action-buttons">
                                <button style="background-color:#8cc861;border-radius:20px;--mdc-ripple-fg-size:53px;--mdc-ripple-fg-scale:2.00702;--mdc-ripple-fg-translate-start:-11.1041px, -12.3959px;--mdc-ripple-fg-translate-end:18.1979px, -8.5px;margin-right: 20px;margin-left: 20px;padding-left: 16px;width: 412px;padding-right: 16px;height: 40px;" tabindex="0" name="login" id="kc-login" type="submit" class="mdc-button mdc-button--raised mdc-card__action">
                                    ${msg("doLogIn")}
                                </button>
                            </div>
                        </div>
    ...
    <#elseif section = "info" >
        <#if realm.password && realm.registrationAllowed && !registrationDisabled??>
            ...
            <div id="kc-registration" class="registration-label-mdc">
                <span>${msg("noAccount")} <a tabindex="0" href="http://angular.signatury.com/user/signuptest">${msg("doRegister")}</a></span>
            </div>
        </#if>
    </#if>
